...
class SocialAccount(models.Model):
    user = models.ForeignKey(allauth.app_settings.USER_MODEL, on_delete=lambda x:x)
    provider = models.CharField(max_length=30,
                                choices=providers.registry.as_choices())
...
class SocialToken(models.Model):
    app = models.ForeignKey(SocialApp, on_delete=lambda x:x)
    account = models.ForeignKey(SocialAccount, on_delete=lambda x:x)
    token = models \
