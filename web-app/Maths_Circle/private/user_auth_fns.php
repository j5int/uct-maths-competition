...
// else return false
{
  ...
  // check if username is unique
  $result = mysql_query("select * from user where username='$username' and password = old_password('$password')");
  if (!$result)
     return 0;
  ...
  if (mysql_num_rows($result)>0)
  {
	$result2 = mysql_query("update user set counter = counter+1, last='$today' where username='$username' and password = old_password('$password')");
	$result3 = mysql_query("select uid from user where username='$username' and password = old_password('$password')");
